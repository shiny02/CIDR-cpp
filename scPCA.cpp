...
void cpp_dist(arma::mat dist, arma::mat truth, arma::mat counts, int ncol, double threshold) {
    ...
    // Work on parallelizing later
    for (int i = 0; i < ncol; i++)
    {
      arma::vec count_col_1 = counts.col(i);
      arma::vec truth_col_1 = truth.col(i);
      for (int j = 0; j < ncol; j++)
      {
        arma::vec count_col_2 = counts.col(j);
        arma::vec truth_col_2 = truth.col(j);
        dist(j, i) = my_accumulate(count_col_1.begin(), count_col_1.end(), count_col_2.begin(), truth_col_1.begin(), truth_col_2.begin(), threshold);
      }
    }
